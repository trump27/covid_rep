...
<script async src="https://www.googletagmanager.com/gtag/js?id=UA-167736534-1"></script>
<script>
  window.dataLayer = window.dataLayer || [];
  function gtag(){dataLayer.push(arguments);}
  gtag('js', new Date());

  gtag('config', 'UA-167736534-1');
</script>
...
data %>%
  ...
  # arrange(desc(確定日)) %>%
  ungroup() %>%
  ggplot(aes(x=確定日, y=人数,color=受診都道府県)) +
    geom_line() +
    gghighlight(label_key=受診都道府県) +
    labs(title="宣言解除後", x="", y="感染数") +
